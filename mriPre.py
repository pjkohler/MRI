...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--trdur", metavar="double", type=float, default=0,
         nargs="?", help="TR duration in seconds (default: compute automatically)")
    parser.add_argument(
        "--pre_tr", metavar="int", type=int, default=0,
         nargs="?", help="How many acquisitions to remove (default=0)")
    parser.add_argument(
        "--total_tr", metavar="double", type=int, default=0,
         nargs="?", help="total number of acquisitions to keep (default: all)")
    parser.add_argument(
        "--diffmat", help="EPIs with different matrices? (default: no)",
        action="store_true")
    parser.add_argument(
        "--tfile", metavar="str filename", type=str,default="none",
         nargs="?", help="Full path to time shift file (default: no file, odd-even interleaved") 
    parser.add_argument(
        "--pad_ap", metavar="int", type=int, default=0,
         nargs="?", help="Number of voxels to pad matrices in a/p direction (default: 0)")
    parser.add_argument(
        "--pad_is", metavar="int", type=int, default=0,
         nargs="?", help="Number of voxels to pad matrices in i/s direction (default: 0)")
    parser.add_argument(
